...
/// Server configuration can't be generated by hand. Using the `http` feature, this is returned
/// from `crate::handshake::http::upgrade_request`, and can be handed to [`Server::configure`]
/// in order to configure the server instance appropriately based on the request headers.
pub struct ServerConfiguration {
	pub(super) extension_config: Vec<String>,
}
...
impl<'a, T: AsyncRead + AsyncWrite + Unpin> Server<'a, T> {
	...
	/// is handled automatically if using [`Server::receive_request`] to handle the
	/// incoming request.
	pub fn configure(&mut self, configuration: ServerConfiguration) -> Result<(), Error> {
		for config_str in configuration.extension_config {
			configure_extensions(&mut self.extensions, &config_str)?;
		}
		Ok(())
	}
